...
# Get the closer MEC service (registration API and port) from the central <domain>
def discover_sb_service(tile,serviceName):
    headers = security.get_header_with_token()
    discovery_url="http://<domain>/discovery-api"
...
# Register a new dataflow and get ID, the AMQP topic to use and the response to the keep-alive message 
...
# This keep-alive method will update dataflow Info in order to tell infrastructure that given dataflow is still available
